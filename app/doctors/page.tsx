...
const SPECIALTIES = [
  "All Specialties","Orthopedic","Cardiology","Nephrology","Neurology",
  "Neuro Surgery","General Surgery","General Medicine","General Physician",
  "Urology","Gynaecology","Pediatrics","Pulmonology","Plastic Surgery",
  "ENT","Physiotherapy","Anesthesia","Pathology","Dermatology",
  "Psychiatry","Radiology","Critical Care","Dental",
];
...
interface Doctor {
  id: string; name: string; slug: string; initials: string; color: string;
  department: string; specialty: string; qualifications: string; image?: string;
  available: boolean; nextSlot: string; rating: number; reviews: number;
  experience: string; patients: string; fee: string; branch: string;
  branchId: string; languages: string[]; about: string;
...
function getInitials(name: string): string {
  return name.replace(/^Dr\.?\s*/i,"").split(/\s+/).filter(Boolean)
...
const DOCTORS: Doctor[] = rawDoctors.map((d, idx) => {
  ...
  return {
    id: d.id, name: d.name, slug: d.slug, initials: getInitials(d.name), color,
    department: d.department, specialty: d.designation, qualifications: d.qualification,
    image: d.image, available,
    ...
    branch: d.branch, branchId, languages: ["Telugu","English"],
  ...
  return (
    <div onClick={onClick} onMouseEnter={()=>setHovered(true)} onMouseLeave={()=>setHovered(false)}
      ...
      <div style={{ padding:"18px 18px 16px" }}>
        <div style={{ display:"inline-flex", alignItems:"center", gap:5, fontSize:10, fontWeight:700, padding:"3px 10px", borderRadius:20, marginBottom:14,
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:8 }}>
          ...
          <button onClick={(e)=>{ e.stopPropagation(); onClick(); }} style={{
            ...
            boxShadow: hovered ? `0 4px 16px ${doctor.color}60` : "none", transition:"all 0.3s ease" }}>Book Now</button>
        </div>
      </div>
    </div>
  );
...
function DoctorDetail({ doctor, onBack }: { doctor:Doctor; onBack:()=>void }) {
  ...
  /* ── window width for responsive switches ── */
  ...
  return (
    <div id="detail-scroll-container" style={{ minHeight:"100vh", overflowY:"auto", background:"#0a0f0f", fontFamily:"'DM Sans',sans-serif" }}>
      ...
      <div style={{
        ...
        transition:"all 0.4s ease", flexWrap:"wrap", gap:8,
      }}>
        ...
        <div style={{ color:"#fff",fontSize:13,fontWeight:600,opacity:scrollY>120?1:0,transition:"opacity 0.3s",flex:1,textAlign:"center" }}>{doctor.name}</div>
      ...
      {/* ── Hero ── */}
      <div ref={heroRef} style={{ position:"relative", height:"100vh", minHeight:isMobile?500:600, overflow:"hidden", display:"flex", alignItems:"flex-end" }}>
        ...
        {/* Hero text */}
        <div style={{ position:"relative",zIndex:10,padding: isMobile ? "0 18px 40px" : "0 40px 60px",maxWidth:600,opacity:heroOpacity,transform:`translateY(${-parallaxOffset*0.3}px)` }}>
          <div style={{ display:"inline-flex",alignItems:"center",gap:6,
            ...
            padding:"6px 14px",borderRadius:30,fontSize:11,fontWeight:700,marginBottom:20,backdropFilter:"blur(10px)" }}>
          ...
          <h1 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(30px,5vw,62px)",fontWeight:700,color:"#fff",lineHeight:1.1,marginBottom:10,letterSpacing:"-0.02em" }}>{doctor.name}</h1>
          ...
          <div style={{ display:"flex",gap:10,flexWrap:"wrap",marginBottom:32 }}>
            {[{ icon:"star",val:doctor.rating.toString(),lbl:"Rating",color:"#fbbf24" },{ icon:"award",val:doctor.experience,lbl:"Experience",color:"#60a5fa" },{ icon:"users",val:doctor.patients,lbl:"Patients",color:"#34d399" }].map((s)=>(
              <div key={s.lbl} style={{ display:"flex",alignItems:"center",gap:10,background:"rgba(255,255,255,0.05)",backdropFilter:"blur(10px)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:12,padding:"10px 14px" }}>
      ...
      <div style={{ background:"#f5f7f7" }}>
        ...
        <div style={{
          ...
          <div style={{ display:"flex",gap:4 }}>
            {(["overview","education","reviews"] as const).map((tab)=>(
              <button key={tab} onClick={()=>setActiveTab(tab)} style={{ padding: isMobile?"7px 10px":"8px 18px",borderRadius:9,border:"none",cursor:"pointer",fontSize: isMobile?10:12,fontWeight:600,textTransform:"capitalize",background:activeTab===tab?"#0a0f0f":"transparent",color:activeTab===tab?"#fff":"#5a7070",transition:"all 0.2s" }}>{tab}</button>
            ))}
          </div>
          <div style={{ display:"flex",alignItems:"center",gap:8,flexWrap:"wrap" }}>
            {!isMobile && <span style={{ fontSize:13,color:"#5a7070" }}>Consultation:</span>}
            <span style={{ fontSize: isMobile?16:20,fontWeight:800,color:"#0a6e6e" }}>{doctor.fee}</span>
            <button onClick={()=>document.getElementById("booking-widget")?.scrollIntoView({behavior:"smooth"})}
              style={{ background:"linear-gradient(135deg,#C0145C,#880E4F)",color:"#fff",border:"none",borderRadius:10,padding: isMobile?"8px 12px":"10px 22px",fontSize: isMobile?10:12,fontWeight:700,cursor:"pointer",boxShadow:"0 4px 16px rgba(192,20,92,0.35)",whiteSpace:"nowrap" }}>
              {isMobile ? "Book" : "Book Appointment"}
            </button>
          </div>
        </div>

        {/* Content + Sidebar — stacks on mobile */}
        <div style={{
          maxWidth:1200, margin:"0 auto",
          padding: isMobile ? "20px 14px 60px" : "40px 24px 80px",
          display:"flex", flexDirection: isMobile?"column":"row",
          gap: isMobile?20:28, alignItems:"flex-start",
        }}>
          ...
          <div id="booking-widget" style={{ width: isMobile?"100%":300, flexShrink:0, display:"flex", flexDirection:"column", gap:16 }}>
            ...
            <div style={{ background:"#0a0f0f",borderRadius:20,overflow:"hidden",border:"1px solid rgba(255,255,255,0.06)" }}>
              ...
              <div style={{ padding:"16px 18px" }}>
                ...
                {/* 4-col on mobile, 3-col on desktop */}
                <div style={{ display:"grid",gridTemplateColumns: isMobile?"repeat(4,1fr)":"repeat(3,1fr)",gap:6,marginBottom:16 }}>
                  {TIME_SLOTS.map((s)=>(
        ...
        {/* YouTube section */}
        <div style={{ background:"#0a0f0f",padding: isMobile?"48px 16px 60px":"70px 24px 80px",borderTop:"1px solid rgba(255,255,255,0.04)" }}>
          <div style={{ maxWidth:1200,margin:"0 auto" }}>
            <div style={{ marginBottom:40,textAlign:"center" }}>
              ...
              <h2 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(24px,4vw,44px)",fontWeight:700,color:"#fff",lineHeight:1.2,marginBottom:12 }}>
                Srikara <em style={{ color:"#C0145C",fontStyle:"normal" }}>Excellence</em> on Screen
              </h2>
            </div>
            {/* Auto-fill — collapses to 1-col on small screens */}
  ...
  return (
    <a href={`https://www.youtube.com/watch?v=${video.id}`} target="_blank" rel="noopener noreferrer"
      ...
      style={{ display:"block",textDecoration:"none",borderRadius:16,overflow:"hidden",
        ...
        transition:"all 0.35s cubic-bezier(0.34,1.56,0.64,1)",background:"#111819",
...
function DoctorsPageInner() {
  ...
  if (selectedDoctor) {
    return (<><GlobalStyle /><DoctorDetail doctor={selectedDoctor} onBack={()=>router.push("/doctors")} /></>);
  }
  ...
  return (
    <>
      ...
      <div style={{ minHeight:"100vh", fontFamily:"'DM Sans',sans-serif", background:"#f5f7f7" }}>
        ...
        {/* Hero */}
        <section style={{ background:"linear-gradient(135deg,#0a0f0f 0%,#1a0a14 60%,#0f1a1a 100%)",padding:"80px 20px 70px",textAlign:"center",position:"relative",overflow:"hidden" }}>
          <div style={{ position:"absolute",inset:0,opacity:.03,backgroundImage:"linear-gradient(rgba(255,255,255,0.8) 1px,transparent 1px),linear-gradient(90deg,rgba(255,255,255,0.8) 1px,transparent 1px)",backgroundSize:"50px 50px" }} />
          ...
          <div style={{ position:"relative",zIndex:1 }}>
            ...
            <h1 style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:"clamp(32px,6vw,68px)",fontWeight:700,color:"#fff",lineHeight:1.1,marginBottom:16,letterSpacing:"-0.02em" }}>
            ...
            <div style={{ display:"flex",justifyContent:"center",gap:10,flexWrap:"wrap" }}>
              {[{ v:`${DOCTORS.length}+`,l:"Doctors" },{ v:`${DOCTORS.filter((d)=>d.available).length}+`,l:"Available Now" },{ v:"5",l:"Branches" },{ v:`${SPECIALTIES.length-1}`,l:"Specialties" }].map((s)=>(
                <div key={s.l} style={{ background:"rgba(255,255,255,0.05)",backdropFilter:"blur(10px)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:12,padding:"12px 18px",textAlign:"center" }}>
        ...
        {/* Branch tabs */}
        <div style={{ background:"#fff",padding:"18px 16px",borderBottom:"1px solid #e8f0f0" }}>
          <div style={{ maxWidth:1200,margin:"0 auto" }}>
            <div style={{ fontSize:10,fontWeight:800,color:"#9aafaf",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:10 }}>Our Branches</div>
            <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
              {BRANCHES.map((b)=>(
                <button key={b.id} onClick={()=>setActiveBranch(b.id)} style={{ padding:"9px 16px",borderRadius:10,border:`1.5px solid ${activeBranch===b.id?"#C0145C":"#e0eeee"}`,background:activeBranch===b.id?"rgba(192,20,92,0.06)":"#fff",fontSize:12,fontWeight:activeBranch===b.id?700:500,color:activeBranch===b.id?"#C0145C":"#3a5050",cursor:"pointer",transition:"all 0.2s" }}>{b.label}</button>
              ))}
            </div>
          </div>
        </div>
        ...
        <div style={{ background:"#f8fafa",padding:"12px 16px",borderBottom:"1px solid #edf0f0" }}>
          <div style={{ maxWidth:1200,margin:"0 auto" }}>
            <div style={{ display:"flex",gap:8,overflowX:"auto",scrollbarWidth:"none",WebkitOverflowScrolling:"touch" as any }}>
              {SPECIALTIES.map((sp)=>(
        ...
        {/* Grid */}
        <section style={{ maxWidth:1200,margin:"0 auto",padding:"28px 16px 80px" }}>
          ...
          {filtered.length===0 ? (
            <div style={{ textAlign:"center",padding:"80px 24px",background:"#fff",borderRadius:20,border:"2px dashed #e0eeee" }}>
              ...
              <button onClick={()=>{ setActiveBranch("all"); setActiveSpec("All Specialties"); setSearch(""); }}
                style={{ background:"#0a6e6e",color:"#fff",border:"none",borderRadius:10,padding:"12px 24px",fontSize:13,fontWeight:700,cursor:"pointer" }}>Reset Filters</button>
            </div>
          ) : (
            /* auto-fill naturally collapses: 3-col → 2-col → 1-col */
  ...
  return (
    <style>{`
      ...
      *,*::before,*::after { box-sizing:border-box; margin:0; padding:0; }
      html,body { font-family:'DM Sans',sans-serif; background:#f5f7f7; color:#2a3f3f; scroll-behavior:smooth; overflow-x:hidden; }
      ...
      /* hide specialty strip scrollbar everywhere */
