...
const BRANCHES = ["RTC X Roads","Miyapur","LB Nagar","Ameerpet","Kukatpally","Dilsukhnagar","Secunderabad","Begumpet","Uppal"];
...
const C = {
  white: "#FAFCFE", off: "#F0F5FA", panel: "#E4EDF5", border: "#CDD9E5",
  mid: "#7A96B0", text: "#0F2235", muted: "#4A657A",
  blue: "#0B4F8C", sky: "#1565C0", teal: "#00838F",
...
const globalStyle = `
  ...
  *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
  ...
  body{font-family:'Plus Jakarta Sans',sans-serif;background:#FAFCFE;color:#0F2235;overflow-x:hidden;}
  ::-webkit-scrollbar{width:4px;}::-webkit-scrollbar-track{background:#F0F5FA;}::-webkit-scrollbar-thumb{background:#1565C0;border-radius:2px;}
...
function Img({ src, alt, style = {}, caption }: { src: string, alt: string, style?: React.CSSProperties, caption?: string }) {
  ...
  return (
    ...
    >
      ...
      />
      {caption && (
        <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, background: "linear-gradient(transparent,rgba(11,79,140,.72))", padding: "2.5rem 1.2rem .9rem", pointerEvents: "none" }}>
  ...
  return (
    <Reveal>
      <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: "3.5rem", flexWrap: "wrap", gap: "1.5rem" }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: ".8rem", fontSize: ".62rem", letterSpacing: ".22em", textTransform: "uppercase", color: C.sky, fontWeight: 600, marginBottom: ".8rem" }}>
            <span style={{ width: 24, height: 2, background: C.sky, display: "block" }} />
            {label}
          </div>
    ...
    <>
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 900, background: "rgba(250,252,254,.97)", backdropFilter: "blur(14px)", borderBottom: `1px solid ${C.border}`, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 2.5rem", height: 66 }}>
        <a href="/" style={{ display: "flex", alignItems: "center", gap: 12, textDecoration: "none" }}>
          <div style={{ width: 38, height: 38, background: C.blue, borderRadius: 4, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Crimson Pro',serif", fontSize: "1.3rem", fontWeight: 600, color: "#fff" }}>S</div>
          <div>
            <div style={{ fontSize: ".8rem", fontWeight: 700, color: C.blue, letterSpacing: ".04em" }}>Srikara Hospital</div>
            <div style={{ fontSize: ".58rem", color: C.mid, letterSpacing: ".16em", textTransform: "uppercase" }}>Vijayawada Branch</div>
          </div>
        </a>
        <div style={{ display: "flex", alignItems: "center", gap: "2.2rem" }}>
          {[["Specialities","#specialities"],["Doctors","#doctors"],["Facilities","#facilities"],["Appointment","#appointment"]].map(([l,h]) => (
            <a key={l} href={h} style={{ fontSize: ".72rem", color: C.muted, letterSpacing: ".06em", fontWeight: 500, textDecoration: "none" }}>{l}</a>
          ))}
          <a href="#appointment" style={{ background: C.blue, color: "#fff", padding: ".5rem 1.4rem", borderRadius: 3, fontSize: ".72rem", fontWeight: 600, letterSpacing: ".08em", textDecoration: "none" }}>Book Now</a>
        </div>
      </nav>
      ...
      <section id="hero" style={{ display: "flex", minHeight: "88vh", background: C.white, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ flex: "0 0 52%", padding: "5.5rem 3.5rem 5rem", position: "relative", zIndex: 2, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          {/* Blueprint grid */}
          <div style={{ position: "absolute", inset: 0, backgroundImage: `linear-gradient(rgba(11,79,140,.03) 1px,transparent 1px),linear-gradient(90deg,rgba(11,79,140,.03) 1px,transparent 1px)`, backgroundSize: "48px 48px", pointerEvents: "none" }} />
          {/* Accent stripe */}
          <div style={{ position: "absolute", top: 0, left: 0, width: 4, height: "100%", background: `linear-gradient(180deg,${C.blue},${C.teal})` }} />

          <div className="a1" style={{ display: "inline-flex", alignItems: "center", gap: ".7rem", background: "rgba(11,79,140,.07)", border: "1px solid rgba(11,79,140,.14)", borderRadius: 2, padding: ".35rem 1rem", marginBottom: "1.8rem", fontSize: ".62rem", letterSpacing: ".2em", textTransform: "uppercase", color: C.blue, fontWeight: 600, width: "fit-content" }}>
          ...
          <h1 className="a2" style={{ fontFamily: "'Crimson Pro',serif", fontSize: "clamp(3rem,5.5vw,5.5rem)", fontWeight: 300, lineHeight: 1.05, color: C.text, marginBottom: "1.4rem", letterSpacing: "-.01em" }}>
            Srikara<br/>Hospital<br/><em><strong style={{ color: C.blue }}>Vijayawada</strong></em>
          </h1>
          <div className="a2" style={{ width: 56, height: 3, background: C.blue, marginBottom: "1.7rem" }} />
          ...
          <div className="a4" style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
            <a href="#appointment" style={{ display: "inline-flex", alignItems: "center", gap: ".5rem", background: C.blue, color: "#fff", padding: ".85rem 2rem", fontSize: ".78rem", fontWeight: 600, letterSpacing: ".08em", borderRadius: 3, textDecoration: "none" }}>Book Appointment →</a>
            <a href="tel:08662490100" style={{ display: "inline-flex", alignItems: "center", gap: ".5rem", border: `1.5px solid ${C.border}`, color: C.muted, padding: ".85rem 2rem", fontSize: ".78rem", fontWeight: 500, letterSpacing: ".08em", borderRadius: 3, textDecoration: "none" }}>🚨 Emergency</a>
          </div>
        ...
        <div style={{ flex: 1, display: "grid", gridTemplateColumns: "1.2fr 1fr", gridTemplateRows: "1.3fr 1fr", gap: 3, background: "#dce8f0" }}>
          ...
          <div style={{ gridRow: "1/3", position: "relative" }}>
            ...
            <div style={{ position: "absolute", bottom: "1.2rem", left: "1.2rem", background: "rgba(255,255,255,.95)", backdropFilter: "blur(8px)", borderLeft: `3px solid ${C.blue}`, padding: ".75rem 1.1rem", boxShadow: "0 4px 20px rgba(0,0,0,.14)" }}>
              <div style={{ fontFamily: "'Crimson Pro',serif", fontSize: "1.5rem", fontWeight: 600, color: C.blue, lineHeight: 1 }}>280<sup style={{ fontSize: ".9rem" }}>+</sup></div>
      ...
      {/* ── Stats Bar ── */}
      <div style={{ background: C.blue, display: "grid", gridTemplateColumns: "repeat(4,1fr)" }}>
        {[["10+","Years in Vijayawada"],["36+","Specialist Doctors"],["55k+","Patients Treated"],["280+","Hospital Beds"]].map(([v,l]) => (
          <div key={l} style={{ padding: "1.6rem 2rem", borderRight: "1px solid rgba(255,255,255,.12)" }}>
            <div style={{ fontFamily: "'Crimson Pro',serif", fontSize: "2.5rem", fontWeight: 600, color: "#fff", lineHeight: 1 }}>{v}</div>
            <div style={{ fontSize: ".6rem", color: "rgba(255,255,255,.55)", letterSpacing: ".14em", textTransform: "uppercase", marginTop: ".4rem" }}>{l}</div>
          </div>
        ))}
      </div>
        ...
        <div className="live-ticker" style={{ display: "inline-flex", gap: "3rem" }}>
          {[...TICKERS,...TICKERS,...TICKERS].map((t, i) => (
            <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: ".8rem", fontSize: ".62rem", letterSpacing: ".18em", textTransform: "uppercase", color: C.mid }}>
              <span style={{ width: 4, height: 4, background: C.blue, borderRadius: "50%", display: "inline-block" }} />{t}
            </span>
          ))}
        </div>
      </div>
      ...
      <section id="specialities" style={{ background: C.white, padding: "6rem 2.5rem" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <SectionHeader label="6 Departments" title='Our <em style="font-style:italic;color:#0B4F8C;">Specialities</em>' />
          <Reveal>
            <div style={{ display: "flex", border: `1px solid ${C.border}`, borderRadius: 4, overflow: "hidden" }}>
              ...
              <div style={{ width: 210, flexShrink: 0, borderRight: `1px solid ${C.border}`, display: "flex", flexDirection: "column" }}>
                {SPECIALITIES.map((s, i) => (
                  <button key={s.id} onClick={() => setActiveSpec(i)}
                    style={{ border: "none", background: activeSpec === i ? "rgba(11,79,140,.05)" : "transparent", borderLeft: `3px solid ${activeSpec === i ? C.blue : "transparent"}`, borderBottom: `1px solid ${C.border}`, padding: "1.2rem 1.4rem", textAlign: "left", cursor: "pointer", transition: "all .25s" }}>
                    <div style={{ fontSize: ".58rem", color: C.sky, letterSpacing: ".16em", marginBottom: ".3rem" }}>{s.id}</div>
              ...
              <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
                <div style={{ flex: 1, padding: "3rem", position: "relative", overflow: "hidden", background: C.white }}>
                  <div style={{ position: "absolute", top: -30, right: -10, fontFamily: "'Crimson Pro',serif", fontSize: "14rem", fontWeight: 600, color: "rgba(11,79,140,.04)", lineHeight: 1, pointerEvents: "none", userSelect: "none" }}>{SPECIALITIES[activeSpec].id}</div>
                  ...
                  <div style={{ width: 40, height: 2, background: C.blue, marginBottom: "1.2rem" }} />
                  <p style={{ fontSize: ".88rem", color: C.muted, lineHeight: 1.85, maxWidth: 420, marginBottom: "2rem" }}>{SPECIALITIES[activeSpec].desc}</p>
                  <a href="#appointment" style={{ display: "inline-flex", alignItems: "center", gap: ".5rem", border: `1.5px solid ${C.blue}`, color: C.blue, padding: ".65rem 1.6rem", fontSize: ".72rem", fontWeight: 600, letterSpacing: ".1em", borderRadius: 3, textDecoration: "none" }}>Book Consultation →</a>
                </div>
      ...
      <section id="doctors" style={{ background: C.off, padding: "6rem 2.5rem" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <SectionHeader label="Our Physicians" title='Expert <em style="font-style:italic;color:#0B4F8C;">Medical Team</em>'
            right={<a href="#" style={{ fontSize: ".7rem", letterSpacing: ".12em", textTransform: "uppercase", color: C.sky, borderBottom: `1px solid rgba(21,101,192,.3)`, paddingBottom: 2, textDecoration: "none" }}>All 36 Specialists →</a>} />
  ...
  <Reveal key={d.name} delay={i * 60}>
    <div
      ...
      style={{
        ...
        borderTop: `3px solid ${hoveredDoc === i ? C.teal : "transparent"}`,
        overflow: "hidden",
        cursor: "pointer",
        transition: "all .3s",
        boxShadow:
          hoveredDoc === i
            ? "0 6px 24px rgba(11,79,140,.1)"
            : "none",
      }}
    >
      <div style={{ height: 220, position: "relative" }}>
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              "linear-gradient(transparent 50%,rgba(11,79,140,.55))",
            pointerEvents: "none",
          }}
        />
      ...
      <section style={{ background: C.off, padding: "6rem 2.5rem" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: "3rem" }}>
            <Reveal>
              <div style={{ display: "flex", alignItems: "center", gap: ".8rem", fontSize: ".62rem", letterSpacing: ".22em", textTransform: "uppercase", color: C.sky, fontWeight: 600, marginBottom: ".8rem", justifyContent: "center" }}>
                <span style={{ width: 24, height: 2, background: C.sky, display: "block" }} />Patient Voices<span style={{ width: 24, height: 2, background: C.sky, display: "block" }} />
              </div>
              <h2 style={{ fontFamily: "'Crimson Pro',serif", fontSize: "clamp(2rem,4vw,3.5rem)", fontWeight: 300, color: C.text, lineHeight: 1.1 }}>
                Stories of <em style={{ fontStyle: "italic", color: C.blue }}>Healing</em>
              </h2>
            </Reveal>
          </div>
          <Reveal>
            <div style={{ maxWidth: 820, margin: "0 auto", background: C.white, border: `1px solid ${C.border}`, padding: "3.5rem", position: "relative", boxShadow: "0 2px 24px rgba(11,79,140,.07)" }}>
              <div style={{ position: "absolute", top: -14, left: "50%", transform: "translateX(-50%)", background: C.blue, color: "#fff", fontSize: ".6rem", letterSpacing: ".18em", textTransform: "uppercase", fontWeight: 600, padding: ".28rem 1.2rem", borderRadius: 2, whiteSpace: "nowrap" }}>{TESTIMONIALS[activeTesti].dept}</div>
              <div style={{ fontFamily: "'Crimson Pro',serif", fontSize: "5rem", color: C.blue, opacity: .1, lineHeight: 1, marginBottom: ".5rem" }}>"</div>
              <p style={{ fontFamily: "'Crimson Pro',serif", fontSize: "clamp(1.1rem,2vw,1.4rem)", fontStyle: "italic", color: C.text, lineHeight: 1.75, marginBottom: "2rem" }}>{TESTIMONIALS[activeTesti].text}</p>
              <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "1rem" }}>
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: ".6rem", letterSpacing: ".12em", textTransform: "uppercase", color: C.sky, marginTop: 3 }}>{TESTIMONIALS[activeTesti].area}</div>
                </div>
                <div style={{ width: 32, height: 1, background: C.border }} />
              </div>
            </div>
            <div style={{ display: "flex", justifyContent: "center", gap: ".5rem", marginTop: "2rem" }}>
              {TESTIMONIALS.map((_, i) => (
                <button key={i} onClick={() => setActiveTesti(i)} style={{ width: i === activeTesti ? 36 : 24, height: 2, background: i === activeTesti ? C.blue : C.border, border: "none", cursor: "pointer", transition: "all .35s", padding: 0 }} />
              ))}
            </div>
          </Reveal>
        </div>
      ...
      <section id="facilities" style={{ background: C.white, padding: "6rem 2.5rem" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto" }}>
          <SectionHeader label="Infrastructure" title='World-Class <em style="font-style:italic;color:#0B4F8C;">Facilities</em>' />
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 1, background: C.border }}>
            {FACILITIES.map((f, i) => (
      ...
      {/* ── Appointment ── */}
      <section id="appointment" style={{ background: C.blue, padding: "6rem 2.5rem" }}>
        <div style={{ maxWidth: 1000, margin: "0 auto" }}>
          <Reveal>
            <div style={{ marginBottom: "2.5rem" }}>
              <div style={{ display: "flex", alignItems: "center", gap: ".7rem", fontSize: ".62rem", letterSpacing: ".22em", textTransform: "uppercase", color: "rgba(255,255,255,.6)", marginBottom: ".8rem" }}>
                <span style={{ width: 20, height: 2, background: "rgba(255,255,255,.4)", display: "block" }} />Schedule a Visit
              </div>
              ...
              <p style={{ fontSize: ".88rem", color: "rgba(255,255,255,.6)" }}>Confirmed within 30 minutes · Walk-ins always welcome</p>
            </div>
          </Reveal>
          <Reveal delay={100}>
            <form onSubmit={handleSubmit}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, background: "rgba(255,255,255,.12)", marginBottom: 1 }}>
                {[{l:"Full Name",k:"name",t:"text",ph:"Your full name"},{l:"Mobile Number",k:"phone",t:"tel",ph:"+91 98765 43210"}].map(f => (
                  <div key={f.k} style={{ background: "rgba(255,255,255,.07)", padding: "1.6rem 1.8rem" }}>
                    <label style={{ fontSize: ".58rem", letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(255,255,255,.65)", display: "block", marginBottom: ".7rem" }}>{f.l}</label>
                    <input type={f.t} placeholder={f.ph} value={form[f.k as "name" | "phone"]} onChange={e => setForm(p => ({ ...p, [f.k]: e.target.value }))}
                      style={{ width: "100%", background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,.2)", color: "#fff", fontSize: "1rem", padding: ".5rem 0", outline: "none", fontFamily: "'Plus Jakarta Sans',sans-serif" }} />
                  </div>
                ))}
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 1, background: "rgba(255,255,255,.12)", marginBottom: 1 }}>
                <div style={{ background: "rgba(255,255,255,.07)", padding: "1.6rem 1.8rem" }}>
                  <label style={{ fontSize: ".58rem", letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(255,255,255,.65)", display: "block", marginBottom: ".7rem" }}>Department</label>
                  <select value={form.dept} onChange={e => setForm(p => ({ ...p, dept: e.target.value }))}
                    style={{ width: "100%", background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,.2)", color: "#fff", fontSize: "1rem", padding: ".5rem 0", outline: "none", fontFamily: "'Plus Jakarta Sans',sans-serif", colorScheme: "dark" }}>
                    <option value="">Select department</option>
                    {SPECIALITIES.map(s => <option key={s.name} style={{ background: C.blue }}>{s.name}</option>)}
                    <option style={{ background: C.blue }}>General Medicine</option>
                    <option style={{ background: C.blue }}>Paediatrics</option>
                  </select>
                </div>
                <div style={{ background: "rgba(255,255,255,.07)", padding: "1.6rem 1.8rem" }}>
                  <label style={{ fontSize: ".58rem", letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(255,255,255,.65)", display: "block", marginBottom: ".7rem" }}>Preferred Date</label>
                  <input type="date" value={form.date} onChange={e => setForm(p => ({ ...p, date: e.target.value }))} min={new Date().toISOString().split("T")[0]}
                    style={{ width: "100%", background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,.2)", color: "#fff", fontSize: "1rem", padding: ".5rem 0", outline: "none", fontFamily: "'Plus Jakarta Sans',sans-serif", colorScheme: "dark" }} />
                </div>
              </div>
              <div style={{ background: "rgba(255,255,255,.07)", padding: "1.6rem 1.8rem", marginBottom: 1 }}>
                <label style={{ fontSize: ".58rem", letterSpacing: ".2em", textTransform: "uppercase", color: "rgba(255,255,255,.65)", display: "block", marginBottom: ".7rem" }}>Reason for Visit</label>
                <input type="text" placeholder="Briefly describe your concern..." value={form.msg} onChange={e => setForm(p => ({ ...p, msg: e.target.value }))}
                  style={{ width: "100%", background: "transparent", border: "none", borderBottom: "1px solid rgba(255,255,255,.2)", color: "#fff", fontSize: "1rem", padding: ".5rem 0", outline: "none", fontFamily: "'Plus Jakarta Sans',sans-serif" }} />
              </div>
              <button type="submit" style={{ width: "100%", background: sent ? "#43A047" : "#fff", color: sent ? "#fff" : C.blue, border: "none", padding: "1.4rem", fontFamily: "'Plus Jakarta Sans',sans-serif", fontSize: ".82rem", fontWeight: 700, letterSpacing: ".16em", textTransform: "uppercase", cursor: "pointer", transition: "all .3s" }}>
      ...
      <section id="contact" style={{ padding: "6rem 2.5rem", background: C.white }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", gap: "5rem", flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "0 0 38%", minWidth: 260 }}>
            <Reveal>
              <div style={{ display: "flex", alignItems: "center", gap: ".8rem", fontSize: ".62rem", letterSpacing: ".22em", textTransform: "uppercase", color: C.sky, fontWeight: 600, marginBottom: "1.5rem" }}>
                <span style={{ width: 24, height: 2, background: C.sky, display: "block" }} />Vijayawada Branch
              </div>
              <h2 style={{ fontFamily: "'Crimson Pro',serif", fontSize: "clamp(1.8rem,3vw,3rem)", fontWeight: 300, color: C.text, marginBottom: "2.5rem", lineHeight: 1 }}>Find Us<br/><em style={{ fontStyle: "italic", color: C.blue }}>Anytime</em></h2>
              {[
                { l:"Address",v:"Plot 14-B, MG Road\nVijayawada – 520 010, AP",col:C.blue },
                { l:"Emergency 24/7",v:"0866-2490-100",col:C.red },
                { l:"Appointments",v:"0866-2490-200",col:C.blue },
                { l:"Email",v:"vijayawada@srikarahospitals.in",col:C.sky },
          ...
          <div style={{ flex: 1 }}>
            <Reveal>
              <div style={{ display: "flex", alignItems: "center", gap: ".8rem", fontSize: ".62rem", letterSpacing: ".22em", textTransform: "uppercase", color: C.sky, fontWeight: 600, marginBottom: "1.5rem" }}>
                <span style={{ width: 24, height: 2, background: C.sky, display: "block" }} />Our Network
              </div>
              <h2 style={{ fontFamily: "'Crimson Pro',serif", fontSize: "clamp(1.8rem,3vw,3rem)", fontWeight: 300, color: C.text, marginBottom: "2.5rem", lineHeight: 1 }}>Srikara<br/><em style={{ fontStyle: "italic", color: C.blue }}>Branches</em></h2>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 1, background: C.border }}>
                {BRANCHES.map(b => (
                  <a key={b} href={`/hospitals/${b.toLowerCase().replace(/ /g, "-")}`} style={{ background: C.off, padding: ".85rem 1.1rem", display: "flex", alignItems: "center", justifyContent: "space-between", fontSize: ".75rem", color: C.muted, fontWeight: 500, textDecoration: "none", borderLeft: "2px solid transparent", transition: "all .2s" }}
                    onMouseEnter={e => { e.currentTarget.style.background = C.panel; e.currentTarget.style.borderLeftColor = C.blue; e.currentTarget.style.color = C.text; }}
                    onMouseLeave={e => { e.currentTarget.style.background = C.off; e.currentTarget.style.borderLeftColor = "transparent"; e.currentTarget.style.color = C.muted; }}>
                    <span>{b}</span><span style={{ fontSize: ".68rem", color: C.sky, opacity: .6 }}>→</span>
                  </a>
                ))}
              </div>
              <a href="/" style={{ display: "inline-flex", alignItems: "center", gap: ".5rem", marginTop: "1.5rem", fontSize: ".68rem", letterSpacing: ".12em", textTransform: "uppercase", color: C.mid, borderBottom: `1px solid ${C.border}`, paddingBottom: 2, textDecoration: "none" }}>← Back to Main Site</a>
            </Reveal>
          </div>
        </div>
      </section>
      ...
      {/* ── Footer ── */}
      <footer style={{ background: C.text, borderTop: `3px solid ${C.blue}`, padding: "1.8rem 2.5rem" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "1rem" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 28, height: 28, background: "rgba(255,255,255,.1)", borderRadius: 3, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Crimson Pro',serif", fontSize: "1rem", fontWeight: 600, color: "#fff" }}>S</div>
            <span style={{ fontSize: ".62rem", color: "rgba(255,255,255,.38)", letterSpacing: ".08em" }}>© {new Date().getFullYear()} Srikara Hospital Vijayawada · Part of Srikara Hospital Group</span>
          </div>
          <div style={{ display: "flex", gap: "2rem" }}>
            {["Privacy","Terms","Sitemap"].map(l => (
              <a key={l} href="#" style={{ fontSize: ".6rem", color: "rgba(255,255,255,.22)", letterSpacing: ".1em", textTransform: "uppercase", textDecoration: "none" }}>{l}</a>
            ))}
          </div>
        </div>
      </footer>
...
function FacCard({ f } : { f: typeof FACILITIES[number] }) {
  ...
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{ background: hov ? "#F0F5FA" : "#FAFCFE", borderBottom: `3px solid ${hov ? "#0B4F8C" : "transparent"}`, overflow: "hidden", cursor: "pointer", transition: "all .3s" }}>
      <div style={{ height: 160, overflow: "hidden", background: "#dce8f0", position: "relative" }}>
        <img src={f.img} alt={f.name} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block", transition: "transform .55s ease", transform: hov ? "scale(1.04)" : "scale(1)" }} />
      </div>
      <div style={{ padding: "1.5rem" }}>
        <span style={{ fontSize: ".58rem", letterSpacing: ".16em", textTransform: "uppercase", border: "1px solid #CDD9E5", color: "#7A96B0", padding: ".22rem .7rem", borderRadius: 2, display: "inline-block", marginBottom: ".9rem" }}>{f.tag}</span>
        <div style={{ fontFamily: "'Crimson Pro',serif", fontSize: "1rem", fontWeight: 600, color: "#0F2235", lineHeight: 1.35, marginBottom: ".6rem" }}>{f.name}</div>
        <div style={{ fontSize: ".76rem", color: "#4A657A", lineHeight: 1.75 }}>{f.desc}</div>
      </div>
    </div>
  );
}
