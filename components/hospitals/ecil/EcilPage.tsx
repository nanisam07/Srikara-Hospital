...
const OTHER_BRANCHES = ["RTC X Roads", "Miyapur", "LB Nagar", "Ameerpet", "Kukatpally", "Dilsukhnagar", "Vijayawada", "Secunderabad"];
...
function useReveal(threshold = 0.1) {
  ...
  useEffect(() => {
    ...
    const io = new IntersectionObserver(([e]) => { if (e.isIntersecting) { setOn(true); io.unobserve(el); } }, { threshold });
...
export default function EcilPage() {
  ...
  const C = { blue: "#0A3D6B", teal: "#00897B", tealLight: "#00ACC1", sky: "#E3F2FD", mist: "#F0F7FF", border: "#E2EAF4", text: "#1A2B3C", mid: "#5A7080", light: "#8FA8BC", white: "#FFFFFF", };
    ...
    <>
      <style>{`
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}
        ...
        body{font-family:'Plus Jakarta Sans',sans-serif;background:#FFFFFF;color:#1A2B3C;overflow-x:hidden;}
        ::-webkit-scrollbar{width:5px;} ::-webkit-scrollbar-track{background:#F0F7FF;} ::-webkit-scrollbar-thumb{background:#0A3D6B;border-radius:3px;}
        ...
        .spec-card:hover{transform:translateY(-6px);box-shadow:0 24px 64px rgba(10,61,107,.15);}
        ...
        .doc-card:hover{transform:translateY(-5px);box-shadow:0 20px 50px rgba(10,61,107,.12);}
        ...
        .fac-card:hover{transform:translateY(-4px);box-shadow:0 16px 48px rgba(10,61,107,.1);}
        .fac-card img{transition:transform .5s ease;}
        .fac-card:hover img{transform:scale(1.05);}

        .btn-primary{display:inline-flex;align-items:center;gap:8px;background:#0A3D6B;color:#fff;padding:.85rem 2rem;border-radius:4px;text-decoration:none;font-size:.85rem;font-weight:700;letter-spacing:.04em;transition:all .25s;cursor:pointer;border:none;font-family:'Plus Jakarta Sans',sans-serif;}
        .btn-primary:hover{background:#0D4F8A;transform:translateY(-2px);box-shadow:0 8px 28px rgba(10,61,107,.3);}
        .btn-outline{display:inline-flex;align-items:center;gap:8px;background:transparent;color:#0A3D6B;padding:.85rem 2rem;border-radius:4px;text-decoration:none;font-size:.85rem;font-weight:600;letter-spacing:.04em;transition:all .25s;border:1.5px solid #0A3D6B;}
        .btn-outline:hover{background:#0A3D6B;color:#fff;}
        .btn-teal{display:inline-flex;align-items:center;gap:8px;background:#00897B;color:#fff;padding:.85rem 2rem;border-radius:4px;text-decoration:none;font-size:.85rem;font-weight:700;letter-spacing:.04em;transition:all .25s;cursor:pointer;border:none;font-family:'Plus Jakarta Sans',sans-serif;}
        .btn-teal:hover{background:#00796B;}

        .section-tag{display:inline-flex;align-items:center;gap:.5rem;font-size:.68rem;font-weight:700;letter-spacing:.2em;text-transform:uppercase;color:#00897B;margin-bottom:.8rem;}
        .section-tag::before{content:'';display:block;width:20px;height:2px;background:#00897B;}
        .section-h{font-family:'Merriweather',serif;font-size:clamp(2rem,3.5vw,3rem);font-weight:700;color:#0A3D6B;line-height:1.2;}
        .section-sub{font-size:1rem;color:#5A7080;line-height:1.8;max-width:540px;}

        input,select,textarea{outline:none;font-family:'Plus Jakarta Sans',sans-serif;}
        input:focus,select:focus,textarea:focus{border-color:#00897B !important;}

        .appt-overlay{position:fixed;inset:0;z-index:900;background:rgba(10,61,107,.6);backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:1rem;animation:fadeIn .25s ease;}
        .appt-modal{background:#fff;border-radius:12px;width:100%;max-width:640px;max-height:90vh;overflow-y:auto;box-shadow:0 40px 100px rgba(10,61,107,.25);animation:slideUp .3s ease;}
      ...
      {/* ── EMERGENCY TOP BAR ── */}
      <div style={{ background: C.blue, padding: "6px 2rem", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: ".5rem" }}>
        <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
          <span style={{ display: "flex", alignItems: "center", gap: 6, fontSize: ".68rem", color: "rgba(255,255,255,.7)", letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 500 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "1.5rem" }}>
          <span style={{ fontSize: ".65rem", color: "rgba(255,255,255,.55)", letterSpacing: ".1em" }}>ECIL X Roads, Hyderabad – 500062</span>
          <Link href="/" style={{ fontSize: ".65rem", color: "rgba(255,255,255,.6)", textDecoration: "none", letterSpacing: ".1em", textTransform: "uppercase", fontWeight: 600 }}>← Main Site</Link>
        </div>
      </div>

      {/* ── NAVBAR ── */}
      <nav style={{ position: "sticky", top: 0, zIndex: 500, background: navSolid ? "rgba(255,255,255,.97)" : "#fff", borderBottom: `1px solid ${navSolid ? C.border : C.border}`, boxShadow: navSolid ? "0 2px 24px rgba(10,61,107,.08)" : "none", backdropFilter: "blur(12px)", transition: "all .3s" }}>
        <div className="section-pad" style={{ maxWidth: 1400, margin: "0 auto", padding: "0 2.5rem", height: 68, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <Link href="/" style={{ display: "flex", alignItems: "center", gap: 12, textDecoration: "none" }}>
            <div style={{ width: 42, height: 42, borderRadius: 8, background: `linear-gradient(135deg,${C.blue},${C.teal})`, display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(10,61,107,.25)" }}>
              <span style={{ color: "#fff", fontSize: "1.3rem", fontFamily: "'Merriweather',serif", fontWeight: 700, fontStyle: "italic" }}>S</span>
            </div>
            <div>
              <p style={{ fontSize: "1rem", fontWeight: 800, color: C.blue, lineHeight: 1.1 }}>Srikara Hospital</p>
              <p style={{ fontSize: ".58rem", color: C.teal, letterSpacing: ".18em", textTransform: "uppercase", fontWeight: 600 }}>ECIL Branch · Hyderabad</p>
            </div>
          </Link>
          ...
          <div className="desk" style={{ display: "flex", alignItems: "center", gap: "2rem" }}>
            {NAV.map(l => (
              <Link key={l.href} href={l.href} style={{ fontSize: ".82rem", color: C.mid, fontWeight: 500, textDecoration: "none", transition: "color .2s" }}
          ...
          <button className="mob-show" onClick={() => setMenuOpen(!menuOpen)}
            ...
            {[0, 1, 2].map(i => <span key={i} style={{ width: 22, height: 1.5, background: C.blue, display: "block" }} />)}
          </button>
        </div>

        {menuOpen && (
          <div style={{ background: "#fff", borderTop: `1px solid ${C.border}`, padding: "1.5rem 2rem", display: "flex", flexDirection: "column", gap: "1rem" }}>
            {NAV.map(l => <Link key={l.href} href={l.href} onClick={() => setMenuOpen(false)} style={{ color: C.text, textDecoration: "none", fontSize: ".9rem", fontWeight: 500, padding: ".5rem 0", borderBottom: `1px solid ${C.border}` }}>{l.label}</Link>)}
      ...
      {/* ── HERO ── */}
      <section style={{ background: C.blue, position: "relative", overflow: "hidden", minHeight: "92vh", display: "flex", alignItems: "center" }}>
        ...
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          ...
          <div style={{ position: "absolute", inset: 0, background: `linear-gradient(105deg,${C.blue} 45%,rgba(10,61,107,.7) 75%,rgba(0,137,123,.3) 100%)` }} />
        </div>
        ...
        <div style={{ position: "relative", zIndex: 2, width: "100%", maxWidth: 1400, margin: "0 auto", padding: "5rem 2.5rem" }} className="hero-pad">
          <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }}>
            ...
            {/* Left */}
            <div>
              <div className="ha1" style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(0,172,193,.15)", border: "1px solid rgba(0,172,193,.3)", borderRadius: 100, padding: ".35rem 1rem", marginBottom: "2rem" }}>
                <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#4ADE80", animation: "pulse 2s ease infinite", display: "inline-block" }} />
                <span style={{ fontSize: ".65rem", fontWeight: 700, letterSpacing: ".16em", textTransform: "uppercase", color: "rgba(0,225,210,.9)" }}>NABH Accredited · ECIL, Hyderabad</span>
              </div>
              ...
              <h1 className="ha2" style={{ fontFamily: "'Merriweather',serif", fontSize: "clamp(2.6rem,5vw,4.8rem)", fontWeight: 700, color: "#fff", lineHeight: 1.1, marginBottom: "1.5rem" }}>
                ...
                <span style={{ color: "#63D9D0", fontStyle: "italic" }}>Close to Home.</span>
              </h1>

              <p className="ha3" style={{ fontSize: "1.05rem", color: "rgba(255,255,255,.7)", lineHeight: 1.85, maxWidth: 480, marginBottom: "2.8rem" }}>
              ...
              <div className="ha4" style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                <button className="btn-teal" onClick={() => setApptOpen(true)}>📅 Book Appointment</button>
                <a href="tel:040-2717-6000" className="btn-outline" style={{ color: "rgba(255,255,255,.85)", borderColor: "rgba(255,255,255,.3)" }}
              ...
              <div className="ha5" style={{ display: "flex", gap: "1rem", flexWrap: "wrap", marginTop: "2.5rem" }}>
                {[{ l: "NABH", s: "Accredited" }, { l: "ISO", s: "9001:2015" }, { l: "NABL", s: "Certified Lab" }, { l: "100+", s: "Insurers" }].map(b => (
                  <div key={b.l} style={{ background: "rgba(255,255,255,.08)", border: "1px solid rgba(255,255,255,.12)", borderRadius: 6, padding: ".5rem .9rem", textAlign: "center" }}>
                    <p style={{ fontSize: ".88rem", fontWeight: 800, color: "#fff", lineHeight: 1 }}>{b.l}</p>
                    <p style={{ fontSize: ".58rem", color: "rgba(255,255,255,.5)", letterSpacing: ".1em", textTransform: "uppercase", marginTop: 2 }}>{b.s}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="ha3 hero-img-col" style={{ position: "relative" }}>
              <div style={{ borderRadius: 16, overflow: "hidden", boxShadow: "0 40px 80px rgba(0,0,0,.4)", position: "relative" }}>
                <img src={IMG.heroDoc} alt="Srikara ECIL doctors" style={{ width: "100%", height: 480, objectFit: "cover", objectPosition: "center top", display: "block" }} />
                <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg,transparent 50%,rgba(10,61,107,.85) 100%)" }} />
                <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "2rem 1.8rem" }}>
                  <p style={{ fontSize: ".65rem", fontWeight: 700, letterSpacing: ".2em", textTransform: "uppercase", color: "#63D9D0", marginBottom: ".4rem" }}>Our Team</p>
              ...
              <div style={{ position: "absolute", top: 20, right: -16, background: "#fff", borderRadius: 10, padding: "1rem 1.2rem", boxShadow: "0 12px 40px rgba(10,61,107,.2)", minWidth: 180, animation: "float 4s ease-in-out infinite" }}>
                ...
                <p style={{ fontSize: "1.1rem", fontWeight: 800, color: C.blue, lineHeight: 1 }}>040-2717-6000</p>
                <p style={{ fontSize: ".62rem", color: C.mid, marginTop: 3 }}>Available 24 hours a day</p>
              </div>

              {/* Floating stat chip */}
              <div style={{ position: "absolute", bottom: 60, left: -16, background: C.teal, borderRadius: 10, padding: "1rem 1.2rem", boxShadow: "0 12px 40px rgba(0,137,123,.3)", animation: "float 5s ease-in-out infinite 1s" }}>
                <p style={{ fontSize: "1.6rem", fontWeight: 800, color: "#fff", lineHeight: 1 }}>65k<span style={{ fontSize: ".9rem" }}>+</span></p>
                <p style={{ fontSize: ".6rem", fontWeight: 600, color: "rgba(255,255,255,.75)", letterSpacing: ".12em", textTransform: "uppercase", marginTop: 2 }}>Patients annually</p>
              </div>
            </div>
          </div>
          ...
          <div className="ha6 stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1px", background: "rgba(255,255,255,.1)", marginTop: "4rem", borderRadius: 10, overflow: "hidden" }}>
            {STATS.map((s, i) => (
              <div key={i} style={{ background: "rgba(255,255,255,.06)", padding: "1.8rem 2rem", backdropFilter: "blur(12px)" }}>
                ...
                <p style={{ fontSize: ".7rem", fontWeight: 600, letterSpacing: ".1em", textTransform: "uppercase", color: "rgba(255,255,255,.5)", marginTop: 6 }}>{s.label}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* ── MARQUEE ── */}
      <div style={{ background: C.teal, padding: ".8rem 0", overflow: "hidden", whiteSpace: "nowrap" }}>
        <div style={{ display: "inline-flex", gap: "3rem", animation: "marquee 30s linear infinite" }}>
          {Array(3).fill(["Advanced Cardiac Care", "Robotic Joint Replacement", "Level III NICU", "3T MRI & PET-CT", "24/7 Trauma Centre", "NABH Accredited", "IVF & Fertility Clinic", "NABL Laboratory", "Stroke Unit", "Bariatric Surgery", "Oncology Tumour Board", "Physiotherapy & Rehab"]).flat().map((t, i) => (
            <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: ".9rem", fontSize: ".68rem", fontWeight: 600, letterSpacing: ".18em", textTransform: "uppercase", color: "rgba(255,255,255,.85)" }}>
              <span style={{ width: 4, height: 4, borderRadius: "50%", background: "rgba(255,255,255,.5)", display: "inline-block" }} />{t}
            </span>
          ))}
        </div>
      </div>
          ...
          <div className="about-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "5rem", alignItems: "center" }}>
            ...
            <Reveal>
              <div style={{ position: "relative" }}>
                <img src={IMG.aboutHosp} alt="Srikara ECIL Hospital" style={{ width: "100%", height: 420, objectFit: "cover", borderRadius: 12, display: "block" }} />
                <div style={{ position: "absolute", bottom: -24, right: -24, borderRadius: 10, overflow: "hidden", border: "4px solid #fff", boxShadow: "0 12px 40px rgba(10,61,107,.15)", width: 200 }}>
                  <img src={IMG.waiting} alt="Patient waiting area" style={{ width: "100%", height: 140, objectFit: "cover", display: "block" }} />
                </div>
                {/* Floating badge */}
                <div style={{ position: "absolute", top: 24, left: 24, background: C.blue, borderRadius: 8, padding: ".8rem 1.2rem", boxShadow: "0 8px 24px rgba(10,61,107,.3)" }}>
                  <p style={{ fontFamily: "'Merriweather',serif", fontSize: "1.8rem", fontWeight: 700, color: "#fff", lineHeight: 1 }}>13+</p>
                  <p style={{ fontSize: ".6rem", fontWeight: 600, color: "rgba(255,255,255,.65)", letterSpacing: ".12em", textTransform: "uppercase" }}>Years of care</p>
                </div>
              </div>
            </Reveal>
            ...
            <Reveal delay={150}>
              ...
              <h2 className="section-h" style={{ marginBottom: "1.2rem" }}>
                A hospital built<br />on trust &amp; expertise
              </h2>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", marginBottom: "2.5rem" }}>
                {[
                  ...
                  <div key={f.l} style={{ display: "flex", alignItems: "flex-start", gap: ".9rem", padding: "1rem", background: C.mist, borderRadius: 8, border: `1px solid ${C.border}` }}>
                    ...
                    <div>
                      <p style={{ fontSize: ".88rem", fontWeight: 700, color: C.blue, lineHeight: 1.2 }}>{f.l}</p>
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto" }}>
          <Reveal>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: "3.5rem", flexWrap: "wrap", gap: "1.5rem" }}>
              ...
              <Link href="#" style={{ fontSize: ".8rem", fontWeight: 700, color: C.teal, textDecoration: "none", borderBottom: `1.5px solid ${C.teal}`, paddingBottom: 2 }}>View All Departments →</Link>
            </div>
          </Reveal>
              ...
              <Reveal key={s.name} delay={i * 60}>
                <div className="spec-card" style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}` }}>
                  ...
                  <div style={{ height: 200, overflow: "hidden", borderRadius: "12px 12px 0 0", position: "relative" }}>
                    ...
                    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg,transparent 40%,rgba(10,61,107,.65) 100%)" }} />
                  ...
                  <div style={{ padding: "1.5rem" }}>
                    <h3 style={{ fontFamily: "'Merriweather',serif", fontSize: "1.1rem", fontWeight: 700, color: C.blue, marginBottom: ".6rem", lineHeight: 1.3 }}>{s.name}</h3>
                    <p style={{ fontSize: ".82rem", color: C.mid, lineHeight: 1.75, marginBottom: "1.2rem" }}>{s.desc}</p>
                    <button className="btn-teal" onClick={() => setApptOpen(true)} style={{ fontSize: ".72rem", padding: ".55rem 1.2rem" }}>Book Appointment →</button>
                  </div>
                </div>
              </Reveal>
            ))}
        ...
        <div style={{ maxWidth: 1400, margin: "0 auto" }}>
          <Reveal>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", marginBottom: "3.5rem", flexWrap: "wrap", gap: "1.5rem" }}>
              ...
              <Link href="#" style={{ fontSize: ".8rem", fontWeight: 700, color: C.teal, textDecoration: "none", borderBottom: `1.5px solid ${C.teal}`, paddingBottom: 2 }}>All 45 Doctors →</Link>
            </div>
          </Reveal>
              ...
              <Reveal key={d.name} delay={i * 70}>
                <div className="doc-card" style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}`, overflow: "hidden" }}>
                  ...
                  <div style={{ height: 240, overflow: "hidden", position: "relative" }}>
                    ...
                    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg,transparent 55%,rgba(10,61,107,.7) 100%)" }} />
                    <div style={{ position: "absolute", bottom: "1rem", left: "1rem" }}>
                      <span style={{ background: C.teal, color: "#fff", fontSize: ".6rem", fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase", padding: ".25rem .7rem", borderRadius: 100 }}>{d.exp} experience</span>
                    </div>
                  </div>
                  {/* Info */}
                  <div style={{ padding: "1.4rem" }}>
                    <h3 style={{ fontFamily: "'Merriweather',serif", fontSize: "1rem", fontWeight: 700, color: C.blue, lineHeight: 1.25, marginBottom: ".3rem" }}>{d.name}</h3>
                    <p style={{ fontSize: ".75rem", fontWeight: 700, color: C.teal, marginBottom: ".5rem" }}>{d.role}</p>
      ...
      {/* ── TESTIMONIALS ── */}
      <section style={{ background: C.blue, padding: "7rem 2.5rem", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          <img src={IMG.heroBg} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: .06 }} />
        </div>
        <div style={{ maxWidth: 960, margin: "0 auto", position: "relative", zIndex: 1, textAlign: "center" }}>
          <Reveal>
            <span className="section-tag" style={{ justifyContent: "center", color: "#63D9D0" }}>
              <span style={{ background: "#63D9D0" }} />
              Patient Stories
            </span>
            ...
            <p style={{ fontSize: "1rem", color: "rgba(255,255,255,.6)", marginBottom: "4rem" }}>Real stories from the families we've had the privilege of caring for</p>
          </Reveal>
              ...
              <div key={i} style={{ position: i === activeTest ? "relative" : "absolute", inset: 0, opacity: i === activeTest ? 1 : 0, transform: i === activeTest ? "none" : "translateY(12px)", transition: "all .7s ease", pointerEvents: i === activeTest ? "auto" : "none" }}>
                <div style={{ background: "rgba(255,255,255,.06)", border: "1px solid rgba(255,255,255,.1)", borderRadius: 16, padding: "3rem", backdropFilter: "blur(12px)" }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "1.5rem", flexWrap: "wrap", gap: "1rem" }}>
                    <span style={{ background: `${C.teal}30`, color: "#63D9D0", fontSize: ".65rem", fontWeight: 700, letterSpacing: ".16em", textTransform: "uppercase", padding: ".3rem .9rem", borderRadius: 100, border: "1px solid rgba(0,172,193,.3)" }}>{t.dept}</span>
                    <Stars n={t.rating} />
                  </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "1rem" }}>
                    <div style={{ width: 48, height: 48, borderRadius: "50%", background: `linear-gradient(135deg,${C.teal},${C.blue})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: "1.2rem", flexShrink: 0 }}>
                    ...
                    <div style={{ textAlign: "left" }}>
                      ...
                      <p style={{ fontSize: ".7rem", color: "rgba(255,255,255,.5)", marginTop: 2 }}>{t.area}</p>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: ".5rem", marginTop: "2.5rem" }}>
            {TESTIMONIALS.map((_, i) => (
              <button key={i} onClick={() => setActiveTest(i)}
                style={{ width: i === activeTest ? 32 : 8, height: 3, background: i === activeTest ? "#63D9D0" : "rgba(255,255,255,.2)", border: "none", cursor: "pointer", transition: "all .3s", borderRadius: 2, padding: 0 }} />
            ))}
          </div>
        </div>
      </section>
              ...
              <Reveal key={f.name} delay={i * 60}>
                <div className="fac-card" style={{ background: "#fff", borderRadius: 12, border: `1px solid ${C.border}` }}>
                  <div style={{ height: 200, overflow: "hidden", borderRadius: "12px 12px 0 0", position: "relative" }}>
                    ...
                    <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg,transparent 30%,rgba(10,61,107,.6) 100%)" }} />
                    <span style={{ position: "absolute", top: "1rem", right: "1rem", background: C.teal, color: "#fff", fontSize: ".6rem", fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase", padding: ".25rem .7rem", borderRadius: 100 }}>{f.tag}</span>
                  </div>
                  <div style={{ padding: "1.4rem" }}>
                    <h3 style={{ fontFamily: "'Merriweather',serif", fontSize: "1rem", fontWeight: 700, color: C.blue, marginBottom: ".6rem", lineHeight: 1.3 }}>{f.name}</h3>
      ...
      <section style={{ background: "#EF4444", position: "relative", overflow: "hidden" }}>
        <div style={{ position: "absolute", inset: 0, zIndex: 0 }}>
          <img src={IMG.ambulance} alt="" style={{ width: "100%", height: "100%", objectFit: "cover", opacity: .12 }} />
        </div>
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "3rem 2.5rem", position: "relative", zIndex: 1, display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "2rem" }}>
          ...
          <a href="tel:040-2717-6000" style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "#fff", color: "#EF4444", padding: "1rem 2.2rem", borderRadius: 6, textDecoration: "none", fontSize: ".9rem", fontWeight: 800, letterSpacing: ".06em", whiteSpace: "nowrap", transition: "all .25s", boxShadow: "0 8px 24px rgba(0,0,0,.2)" }}
      ...
      <section id="contact" className="section-pad" style={{ background: "#fff", padding: "7rem 2.5rem" }}>
        <div style={{ maxWidth: 1400, margin: "0 auto" }}>
          ...
          <div className="contact-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "3rem" }}>
                ...
                <div style={{ padding: "2.5rem" }}>
                  {[
                    ...
                    <div key={c.l} style={{ display: "flex", gap: "1rem", paddingBottom: "1.2rem", marginBottom: i < 4 ? "1.2rem" : 0, borderBottom: i < 4 ? `1px solid ${C.border}` : "none" }}>
                      ...
                      <div>
                        <p style={{ fontSize: ".6rem", fontWeight: 700, letterSpacing: ".16em", textTransform: "uppercase", color: C.teal, marginBottom: ".3rem" }}>{c.l}</p>
                        <p style={{ fontSize: ".9rem", color: C.blue, fontWeight: 600, whiteSpace: "pre-line", lineHeight: 1.6 }}>{c.v}</p>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <Reveal delay={100}>
              <div>
                <h3 style={{ fontFamily: "'Merriweather',serif", fontSize: "1.5rem", fontWeight: 700, color: C.blue, marginBottom: ".5rem" }}>Our Other Branches</h3>
                ...
                <div className="branch-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "1px", background: C.border, borderRadius: 10, overflow: "hidden", marginBottom: "2rem" }}>
                  {OTHER_BRANCHES.map(b => (
                    <Link key={b} href={`/hospitals/${b.toLowerCase().replace(/ /g, "-")}`}
                      ...
                      <p style={{ fontSize: ".75rem", fontWeight: 600, color: C.blue }}>{b}</p>
                    </Link>
                  ))}
                </div>
                <Link href="/" style={{ display: "inline-flex", alignItems: "center", gap: 8, fontSize: ".8rem", fontWeight: 700, color: C.teal, textDecoration: "none", borderBottom: `1.5px solid ${C.teal}`, paddingBottom: 2 }}>
      ...
      {/* ── FOOTER ── */}
      <footer style={{ background: C.blue }}>
        <div style={{ height: 3, background: `linear-gradient(90deg,${C.teal},${C.blue},${C.teal})` }} />
        <div style={{ maxWidth: 1400, margin: "0 auto", padding: "2rem 2.5rem" }}>
          <div className="footer-grid" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 36, height: 36, borderRadius: 6, background: "rgba(255,255,255,.1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span style={{ fontFamily: "'Merriweather',serif", fontWeight: 700, fontSize: "1.1rem", fontStyle: "italic", color: "#fff" }}>S</span>
              </div>
              <span style={{ fontSize: ".72rem", color: "rgba(255,255,255,.4)", letterSpacing: ".06em" }}>
                © {new Date().getFullYear()} Srikara Hospital – ECIL Branch · Part of Srikara Hospital Group, Hyderabad
              </span>
            </div>
            <div style={{ display: "flex", gap: "2rem" }}>
              {["Privacy Policy", "Terms of Service", "Sitemap"].map(l => (
                <Link key={l} href="#" style={{ fontSize: ".65rem", color: "rgba(255,255,255,.3)", textDecoration: "none", letterSpacing: ".08em" }}>{l}</Link>
              ))}
            </div>
          </div>
        </div>
      ...
      <button onClick={() => setApptOpen(true)}
        style={{ position: "fixed", bottom: "2rem", right: "2rem", zIndex: 400, background: C.teal, color: "#fff", border: "none", borderRadius: "50px", padding: ".9rem 1.6rem", fontSize: ".82rem", fontWeight: 700, letterSpacing: ".06em", cursor: "pointer", boxShadow: "0 8px 32px rgba(0,137,123,.45)", transition: "all .3s", display: "flex", alignItems: "center", gap: 8, fontFamily: "'Plus Jakarta Sans',sans-serif" }}
        onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.transform = "scale(1.05)"; (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 12px 40px rgba(0,137,123,.55)"; }}
        onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.transform = "scale(1)"; (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 8px 32px rgba(0,137,123,.45)"; }}>
      ...
      {apptOpen && (
        <div className="appt-overlay" onClick={(e) => { if (e.target === e.currentTarget) setApptOpen(false); }}>
          <div className="appt-modal">
            {/* Modal header */}
            <div style={{ background: `linear-gradient(135deg,${C.blue},${C.teal})`, padding: "2rem 2rem 1.5rem" }}>
              <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
                <div>
                  <p style={{ fontSize: ".65rem", fontWeight: 700, letterSpacing: ".18em", textTransform: "uppercase", color: "rgba(255,255,255,.7)", marginBottom: ".4rem" }}>Srikara Hospital · ECIL Branch</p>
                  <h3 style={{ fontFamily: "'Merriweather',serif", fontSize: "1.5rem", fontWeight: 700, color: "#fff", lineHeight: 1.2 }}>Book Your Appointment</h3>
                  <p style={{ fontSize: ".8rem", color: "rgba(255,255,255,.65)", marginTop: ".4rem" }}>Confirmed within 30 minutes · Mon–Sat, 8 AM – 8 PM</p>
                </div>
                <button onClick={() => setApptOpen(false)} style={{ background: "rgba(255,255,255,.15)", border: "none", borderRadius: "50%", width: 36, height: 36, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", color: "#fff", fontSize: "1.1rem", flexShrink: 0 }}>✕</button>
              </div>
            </div>
            ...
            <form onSubmit={submit} style={{ padding: "2rem" }}>
              ...
              <button type="submit" className="btn-teal" style={{ width: "100%", justifyContent: "center", padding: "1rem", fontSize: ".9rem", background: sent ? "#22c55e" : C.teal }}>
              ...
              <p style={{ textAlign: "center", fontSize: ".72rem", color: C.mid, marginTop: "1rem" }}>
                Or call us directly: <a href="tel:040-2717-6100" style={{ color: C.teal, fontWeight: 700, textDecoration: "none" }}>040-2717-6100</a>
              </p>
            </form>
          </div>
        </div>
